...
export function StickyBar({ settings }: Props) {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center pb-[max(0.5rem,env(safe-area-inset-bottom))] px-2">
      <motion.nav
        className="pointer-events-auto flex w-full max-w-md flex-col items-stretch gap-2 rounded-[1.35rem] border border-ff-glow/22 bg-ff-void/92 px-2.5 py-2.5 ff-shadow-bar backdrop-blur-xl backdrop-saturate-150 sm:px-3"
      ...
      >
        ...
        >
          <Link
            ...
            className="inline-flex min-h-[44px] w-full max-w-xs items-center justify-center rounded-xl bg-gradient-to-r from-ff-glow to-ff-glow-dim px-5 py-2.5 text-sm font-semibold text-ff-void ff-shadow-primary sm:w-auto"
          >
            Book table
          </Link>
        </motion.div>
            ...
            {contactActions.map((a) => (
              <motion.a
                ...
                {...(a.external
                  ...
                  : {})}
                className="inline-flex min-h-[44px] min-w-[44px] flex-1 basis-[calc(50%-0.25rem)] items-center justify-center gap-1.5 rounded-xl border border-ff-mint/22 bg-ff-deep/90 px-2 py-2 text-[11px] font-medium text-ff-mist ff-shadow-soft transition-colors hover:border-ff-glow/35 hover:bg-ff-forest/90 sm:flex-initial sm:basis-auto sm:px-3.5"
                whileHover={hover}
                whileTap={tap}
                transition={spring}
              >
                {a.icon}
                <span className="max-w-[5.5rem] truncate sm:max-w-none">{a.label}</span>
              </motion.a>
            ))}
          </div>
        )}
